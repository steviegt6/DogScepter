...
public static partial class Bytecode
{
    public static void ProcessReferences(CodeContext ctx)
    {
        ...
        foreach (var p in ctx.FunctionPatches)
        {
            ...
            p.Target.Function = new GMCode.Bytecode.Instruction.Reference<GMFunctionEntry>(entry);
        }

        // Handle linking local variables early on
        foreach (var p in ctx.VariablePatches)
        {
            if (p.Token.InstanceType == (int)GMCode.Bytecode.Instruction.InstanceType.Local)
            {
        ...
        if (ctx.BaseContext.NameToCode.TryGetValue(ctx.Name, out GMCode existing))
        {
            ...
            foreach (var instruction in existing.BytecodeEntry.Instructions)
            {
                ...
                {
                    if (instruction.Variable.Target.VariableType == GMCode.Bytecode.Instruction.InstanceType.Local)
        ...
        foreach (var p in ctx.VariablePatches)
        {
            ...
            {
                variable = vari.FindOrDefine(p.Token.Name, (GMCode.Bytecode.Instruction.InstanceType)p.Token.InstanceType, p.Token.Builtin != null, ctx.BaseContext.Project.DataHandle);
            }
            p.Target.Variable = new GMCode.Bytecode.Instruction.Reference<GMVariable>(variable, p.Token.VariableType);
            if (p.Token.VariableType == GMCode.Bytecode.Instruction.VariableType.Normal)
                p.Target.TypeInst = (GMCode.Bytecode.Instruction.InstanceType)p.Token.InstanceType;
        }
    }
    ...
    public static void CompileStatement(CodeContext ctx, Node stmt)
    {
        switch (stmt.Kind)
        {
            ...
            case NodeKind.FunctionCallChain:
                ...
                Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Popz, GMCode.Bytecode.Instruction.DataType.Variable);
                break;
            case NodeKind.Assign:
                switch (stmt.Token.Kind)
                {
                    ...
                    case TokenKind.AssignPlus:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Add);
                        break;
                    case TokenKind.AssignMinus:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Sub);
                        break;
                    case TokenKind.AssignTimes:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Mul);
                        break;
                    case TokenKind.AssignDivide:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Div);
                        break;
                    case TokenKind.AssignAnd:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.And, true);
                        break;
                    case TokenKind.AssignOr:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Or, true);
                        break;
                    case TokenKind.AssignXor:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Xor, true);
                        break;
                    case TokenKind.AssignMod:
                        CompileCompoundAssign(ctx, stmt, GMCode.Bytecode.Instruction.Opcode.Mod);
                        break;
                }
                break;
            case NodeKind.Exit:
                ExitCleanup(ctx);
                Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Exit, GMCode.Bytecode.Instruction.DataType.Int32);
                break;
            case NodeKind.Break:
                ...
                ctx.BytecodeContexts.Peek().UseBreakJump().Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
                break;
            case NodeKind.Continue:
                ...
                ctx.BytecodeContexts.Peek().UseContinueJump().Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
                break;
            case NodeKind.Return:
                ...
                else
                {
                    ...
                    if (ctx.BytecodeContexts.Count != 0)
                    {
                        ...
                        var tempVar = new TokenVariable("$$$$temp$$$$", null) { InstanceType = (int)GMCode.Bytecode.Instruction.InstanceType.Local };
                        EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, tempVar, GMCode.Bytecode.Instruction.DataType.Variable);

                        // Then, clean up outer contexts
                        ExitCleanup(ctx);

                        // Finally, restore the return value to the stack
                        EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.PushLoc, GMCode.Bytecode.Instruction.DataType.Variable, tempVar);
                    }

                    // Actual return instruction
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Ret, GMCode.Bytecode.Instruction.DataType.Variable);
                }
                break;
            case NodeKind.If:
                {
                    ...
                    var conditionJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bf));
                    ...
                    {
                        // Else branch
                        var elseJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
            ...
            case NodeKind.Switch:
                {
                    ...
                    CompileExpression(ctx, stmt.Children[0]);
                    GMCode.Bytecode.Instruction.DataType exprType = ctx.TypeStack.Pop();
                            ...
                            case NodeKind.SwitchCase:
                                {
                                    ...
                                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Dup, exprType);
                                    CompileExpression(ctx, curr.Children[0]);
                                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Cmp, ctx.TypeStack.Pop(), exprType).ComparisonKind = GMCode.Bytecode.Instruction.ComparisonType.EQ;

                                    // Branch to target code if equal (and add to list of cases)
                                    cases.Add(new SwitchCase(new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bt)), i));
                                }
                                break;
                    ...
                    defaultJump?.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
                    endJump.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
                    ...
                    if (bctx.ContinueUsed)
                    {
                        ...
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Popz, exprType);
                        ctx.BytecodeContexts.Peek().UseContinueJump().Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
                    }
            ...
            case NodeKind.With:
                {
                    // Instance
                    CompileExpression(ctx, stmt.Children[0]);
                    GMCode.Bytecode.Instruction.DataType type = ctx.TypeStack.Pop();
                    if (type != GMCode.Bytecode.Instruction.DataType.Int32)
                    {
                        if (type == GMCode.Bytecode.Instruction.DataType.Variable && ctx.BaseContext.IsGMS23)
                        {
                    ...
                    // Body
                    var popEnvJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.PushEnv));
                    ...
                    startJump.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.PopEnv));
                    startJump.Finish(ctx);

                    if (ctx.BytecodeContexts.Pop().BreakUsed)
                    {
                        // Need to generate custom "magic" popenv block
                        var endOfCleanupJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));

                        endJump.Finish(ctx);

                        // Special magic that doesn't continue the loop
                        var instr = Emit(ctx, GMCode.Bytecode.Instruction.Opcode.PopEnv);
            ...
            case NodeKind.While:
                {
                    ...
                    CompileExpression(ctx, stmt.Children[0]);
                    ConvertTo(ctx, GMCode.Bytecode.Instruction.DataType.Boolean);
                    var endLoopJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bf));

                    // Body
                    ctx.BytecodeContexts.Push(new Context(Context.ContextKind.BasicLoop, endLoopJump, repeatJump));
                    CompileStatement(ctx, stmt.Children[1]);
                    ctx.BytecodeContexts.Pop();
                    repeatJump.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
            ...
            case NodeKind.Repeat:
                {
                    ...
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Dup, GMCode.Bytecode.Instruction.DataType.Int32);
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Int32).Value = 0; // special instruction
                    EmitCompare(ctx, GMCode.Bytecode.Instruction.ComparisonType.LTE, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Int32);
                    var endJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bt));
                    var continueJump = new JumpForwardPatch();
                    ...
                    // Decrement counter
                    continueJump.Finish(ctx);
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Int32).Value = 1; // special instruction
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Sub, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Int32);
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Dup, GMCode.Bytecode.Instruction.DataType.Int32);
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Conv, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Boolean);
                    startJump.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bt));
                    startJump.Finish(ctx);

                    // Cleanup
                    endJump.Finish(ctx);
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Popz, GMCode.Bytecode.Instruction.DataType.Int32);
                }
                break;
            case NodeKind.DoUntil:
                {
                    ...
                    CompileExpression(ctx, stmt.Children[1]);
                    ConvertTo(ctx, GMCode.Bytecode.Instruction.DataType.Boolean);
                    startJump.Add(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bf));
                    startJump.Finish(ctx);
            ...
            case NodeKind.FunctionDecl:
                ...
                Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Popz, ctx.TypeStack.Pop());
                break;
    ...
    private static void CompileFunctionCall(CodeContext ctx, Node func, bool inChain)
    {
        ...
        if (funcRef == null && tokenFunc?.Builtin == null && !inChain)
        {
            ...
            string funcName;
            GMCode.Bytecode.Instruction.InstanceType explicitType = GMCode.Bytecode.Instruction.InstanceType.Undefined;
            switch (tokenFunc.ExplicitInstType)
            {
                case GMCode.Bytecode.Instruction.InstanceType.Other:
                    funcName = "@@Other@@";
                    explicitType = GMCode.Bytecode.Instruction.InstanceType.Other;
                    break;
                case GMCode.Bytecode.Instruction.InstanceType.Global:
                    funcName = "@@Global@@";
                    explicitType = GMCode.Bytecode.Instruction.InstanceType.Global;
                    break;
                case GMCode.Bytecode.Instruction.InstanceType.Self:
                    explicitType = GMCode.Bytecode.Instruction.InstanceType.Self;
                    funcName = "@@This@@";
                    break;
                default:
                    funcName = "@@This@@";
            ...
            EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, Builtins.MakeFuncToken(ctx, funcName), 0);
            ...
            { 
                ...
                ExplicitInstType = (explicitType != GMCode.Bytecode.Instruction.InstanceType.Undefined) 
            };
            ...
            Emit(ctx, GMCode.Bytecode.Instruction.Opcode.CallV, GMCode.Bytecode.Instruction.DataType.Variable).Extra = (byte)func.Children.Count;
        ...
        if (ctx.BaseContext.IsGMS23)
        {
            if (inChain)
            {
                ...
                EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Variable, (byte)func.Children.Count, 1);
                EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Variable, 0);

                // Push function ID to stack using variable
                var tokenVar = new TokenVariable(tokenFunc.Name, null);
                tokenVar.InstanceType = (int)GMCode.Bytecode.Instruction.InstanceType.StackTop; // different from other places!
                EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Variable, tokenVar);
            ...
            else
            {
                ...
                EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, tokenFunc, func.Children.Count, token, funcRef);
                ctx.TypeStack.Push(GMCode.Bytecode.Instruction.DataType.Variable);
            }
        }
        else
        ...
        foreach (var context in ctx.BytecodeContexts)
        {
            switch (context.Kind)
            {
                case Context.ContextKind.Switch:
                    ...
                    Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Popz, context.DuplicatedType);
                    break;
    ...
    private static void CompileAssign(CodeContext ctx, Node lhs)
    {
        ...
        if (lhs.Kind == NodeKind.Variable)
        {
            if (lhs.Children.Count == 0)
            {
                ...
                ProcessTokenVariable(ctx, ref tokenVar);
                EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, tokenVar, type);
            }
            else
            {
                // Variable with array
                ...
                if (ctx.BaseContext.IsGMS23)
                {
                    ...
                    {
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Conv, type, GMCode.Bytecode.Instruction.DataType.Variable);
                ...
                {
                    // 2d array or above
                    variable.VariableType = GMCode.Bytecode.Instruction.VariableType.MultiPushPop;
                    if (ctx.BaseContext.IsGMS23)
                        variable.InstanceType = (int)GMCode.Bytecode.Instruction.InstanceType.Self;
                    EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Variable, variable);
                    CompileMultiArrayPop(ctx, lhs);
                }
                else
                {
                    // 1d array
                    variable.VariableType = GMCode.Bytecode.Instruction.VariableType.Array;
                    if (ctx.BaseContext.IsGMS23)
                        variable.InstanceType = (int)GMCode.Bytecode.Instruction.InstanceType.Self;
                    EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, variable, type);
                }
            }
        }
        else if (lhs.Kind == NodeKind.ChainReference)
        {
            ...
            if (end.Children.Count != 0)
            {
                ...
                else
                {
                    if (end.Children.Count >= 2)
                    {
                        ...
                        ConvertTo(ctx, GMCode.Bytecode.Instruction.DataType.Int32);
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Int32).Value = (int)32000;
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Mul, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Int32);
                        CompileExpression(ctx, end.Children[1]); // second index
                        ConvertTo(ctx, GMCode.Bytecode.Instruction.DataType.Int32);
                        EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.chkindex);
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Add, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Int32);
                    }
                    else
                    {
            ...
            else
            {
                ...
                variable.VariableType = GMCode.Bytecode.Instruction.VariableType.StackTop;
                EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, variable, type);
            }
        }
        else
    ...
    private static void CompileCompoundAssign(CodeContext ctx, Node assign, GMCode.Bytecode.Instruction.Opcode opcode, bool bitwise = false)
    {
        ...
        if (lhs.Kind == NodeKind.Variable)
        {
            ...
            if (lhs.Children.Count != 0)
            {
                ...
                if (lhs.Children.Count == 1)
                {
                    ...
                    EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Int32, (byte)1);
            ...
            if (bitwise)
            {
                ConvertTo(ctx, GMCode.Bytecode.Instruction.DataType.Int64);
            ...
            else
            {
                if (type == GMCode.Bytecode.Instruction.DataType.Boolean)
                {
                    ...
                    type = GMCode.Bytecode.Instruction.DataType.Int32;
                }
            }

            // Actual specific operation
            Emit(ctx, opcode, type, GMCode.Bytecode.Instruction.DataType.Variable);

            if (lhs.Children.Count >= 2)
            {
                // Store multi-dimensional array
                EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.restorearef);
                EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, 5);
                EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.popaf);
            }
            else
            {
                // Store result in original variable
                EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, type, variable, GMCode.Bytecode.Instruction.DataType.Variable);
            }
        }
        else if (lhs.Kind == NodeKind.ChainReference)
        {
            ...
            if (end.Children.Count != 0)
            {
                ...
                {
                    // This is a 1d array, so just use dup shenanigans instead of other instructions
                    EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Int32, usedMagic ? (byte)5 : (byte)1);
            ...
            else
            {
                ...
                EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Int32, usedMagic ? (byte)4 : (byte)0);
    ...
    private static void CompilePrePostOperation(CodeContext ctx, Node node)
    {
        ...
        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Push, GMCode.Bytecode.Instruction.DataType.Int16).Value = (short)1;

        // Actual operation
        if (node.Token.Kind == TokenKind.Increment)
            Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Add, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Variable);
        else
            Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Sub, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Variable);
    }
    ...
    private static void CompilePrefixAndPostfix(CodeContext ctx, Node node, bool leaveOnStack, bool isPrefix)
    {
        ...
        if (expr.Kind == NodeKind.Variable)
        {
            ...
            if (expr.Children.Count == 0)
            {
                ...
                EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, variable, GMCode.Bytecode.Instruction.DataType.Variable);
            }
            else
            {
                // Variable with array
                ...
                if (leaveOnStack)
                {
                    ...
                    {
                        if (expr.Children.Count == 1)
                            EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, (byte)6); // 1d array
                        else if (expr.Children.Count >= 2)
                            EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, 9); // multi-dimensional array
                        else
                            EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, (byte)5); // normal variable
                    }
                    else
                    {
                        // pre-2.3 swap instruction
                        Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Int16, GMCode.Bytecode.Instruction.DataType.Variable).TypeInst = (GMCode.Bytecode.Instruction.InstanceType)5;
                    }
                    ctx.TypeStack.Push(GMCode.Bytecode.Instruction.DataType.Variable);
                }
                ...
                {
                    // Store result in original variable
                    EmitVariable(ctx, GMCode.Bytecode.Instruction.Opcode.Pop, GMCode.Bytecode.Instruction.DataType.Variable, variable, GMCode.Bytecode.Instruction.DataType.Int32);
                }
            }
        }
        else if (expr.Kind == NodeKind.ChainReference)
        {
            ...
            if (leaveOnStack)
            {
                ...
                {
                    if (end.Children.Count == 1)
                        EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, usedMagic ? (byte)10 : (byte)6); // 1d array
                    else if (end.Children.Count >= 2)
                        EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, 9); // multi-dimensional array
                    else
                        EmitDupSwap(ctx, GMCode.Bytecode.Instruction.DataType.Int32, 4, usedMagic ? (byte)9 : (byte)5); // normal variable
                }
                else
                {
                    // pre-2.3 swap instruction
    ...
    private static void CompileMultiArrayPrefix(CodeContext ctx, Node variable)
    {
        for (int i = 1; i < variable.Children.Count; i++)
        {
            ...
            if (i == variable.Children.Count - 1)
            {
                ...
                EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Int32, (byte)4);
    ...
    private static void CompileMultiArrayCombined(CodeContext ctx, Node variable)
    {
        for (int i = 1; i < variable.Children.Count; i++)
        {
            ...
            if (i == variable.Children.Count - 1)
            {
                // For the last one, use a dup and pushaf
                EmitDup(ctx, GMCode.Bytecode.Instruction.DataType.Int32, (byte)4);
                EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.savearef);
                EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.pushaf);
            }
            else
            {
    ...
    private static void CompileStatic(CodeContext ctx, Node block)
    {
        ...
        EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.isstaticok);
        var conditionJump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Bt));
        ...
        conditionJump.Finish(ctx);
        EmitBreak(ctx, GMCode.Bytecode.Instruction.BreakType.setstatic);
    ...
    private static void CompileFunctionDecl(CodeContext ctx, Node func)
    {
        ...
        var jump = new JumpForwardPatch(Emit(ctx, GMCode.Bytecode.Instruction.Opcode.B));
        ...
        if (info.InheritingIndex != -1)
        {
            ...
            if (ctx.BaseContext.Functions.TryGetValue((inheritedFuncCall.Token.Value as TokenVariable).Name, out var funcRef))
            {
                ...
                Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Conv, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Variable);
                EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, Builtins.MakeFuncToken(ctx, "@@CopyStatic@@"), 1);
            }
            else
        ...
        if (info.IsConstructor)
        {
            ...
            EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, Builtins.MakeFuncToken(ctx, "@@NullObject@@"), 0);
        }
        else
        {
            // Normal functions either bind to -1 (self) or -16 (static) as a context
            Emit(ctx, GMCode.Bytecode.Instruction.Opcode.PushI, GMCode.Bytecode.Instruction.DataType.Int16).Value = (short)(ctx.InStaticBlock ? -16 : -1);
            Emit(ctx, GMCode.Bytecode.Instruction.Opcode.Conv, GMCode.Bytecode.Instruction.DataType.Int32, GMCode.Bytecode.Instruction.DataType.Variable);
        }

        // Create the method using the context and function ID
        EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, Builtins.MakeFuncToken(ctx, "method"), 2);
        ...
        if (func.Children[0].Kind != NodeKind.Empty)
        {
            ...
            if ((func.Children[0].Token.Value as TokenVariable).InstanceType == (int)GMCode.Bytecode.Instruction.InstanceType.Static)
    ...
    private static void CompileNew(CodeContext ctx, Node stmt)
    {
        ...
        EmitCall(ctx, GMCode.Bytecode.Instruction.Opcode.Call, GMCode.Bytecode.Instruction.DataType.Int32, Builtins.MakeFuncToken(ctx, "@@NewGMLObject@@"), stmt.Children.Count);
        ctx.TypeStack.Push(GMCode.Bytecode.Instruction.DataType.Variable);
    }
}
