...
public interface ASTNode
{
    ...
    public GMCode.Bytecode.Instruction.DataType DataType { get; set; }
    ...
    {
        if (node.DataType != GMCode.Bytecode.Instruction.DataType.Unset)
            return GMCode.Bytecode.Instruction.GetDataTypeStackLength(node.DataType);
        switch (node.Kind)
        {
            ...
            case StatementKind.Binary:
                return GMCode.Bytecode.Instruction.GetDataTypeStackLength((node as ASTBinary).Instruction.Type2);
        }
        return 16;
    }
}
...
public class ASTBlock : ASTNode
{
    ...
    public bool Duplicated { get; set; }
    public bool NeedsParentheses { get; set; }
    public GMCode.Bytecode.Instruction.DataType DataType { get; set; } = GMCode.Bytecode.Instruction.DataType.Unset;
...
public class ASTUnary : ASTNode
{
    ...
    public GMCode.Bytecode.Instruction Instruction;
    public ASTUnary(GMCode.Bytecode.Instruction inst, ASTNode node)
    {
        Instruction = inst;
        Children = new() { node };
    }
    ...
    public void Write(DecompileContext ctx, StringBuilder sb)
    {
        ...
        switch (Instruction.Kind)
        {
            ...
            case GMCode.Bytecode.Instruction.Opcode.Not:
                if (Instruction.Type1 == GMCode.Bytecode.Instruction.DataType.Boolean)
                    sb.Append('!');
                else
                    sb.Append('~');
                break;
...
public class ASTBinary : ASTNode
{
    ...
    public ASTBinary(GMCode.Bytecode.Instruction inst, ASTNode left, ASTNode right)
    {
        Instruction = inst;
    ...
    public void Write(DecompileContext ctx, StringBuilder sb)
    {
        ...
        switch (Instruction.Kind)
        {
            case GMCode.Bytecode.Instruction.Opcode.Mul: op = " * "; break;
            case GMCode.Bytecode.Instruction.Opcode.Div: op = " / "; break;
            case GMCode.Bytecode.Instruction.Opcode.Rem: op = " div "; break;
            case GMCode.Bytecode.Instruction.Opcode.Mod: op = " % "; break;
            case GMCode.Bytecode.Instruction.Opcode.Add: op = " + "; break;
            case GMCode.Bytecode.Instruction.Opcode.Sub: op = " - "; break;
            case GMCode.Bytecode.Instruction.Opcode.And:
                if (Instruction.Type1 == GMCode.Bytecode.Instruction.DataType.Boolean &&
                    Instruction.Type2 == GMCode.Bytecode.Instruction.DataType.Boolean)
            ...
            case GMCode.Bytecode.Instruction.Opcode.Xor:
                if (Instruction.Type1 == GMCode.Bytecode.Instruction.DataType.Boolean &&
                    ...
                    op = " ^ ";
                break;
            case GMCode.Bytecode.Instruction.Opcode.Shl: op = " << "; break;
            case GMCode.Bytecode.Instruction.Opcode.Shr: op = " >> "; break;
            case GMCode.Bytecode.Instruction.Opcode.Cmp:
                op = Instruction.ComparisonKind switch
                {
                    GMCode.Bytecode.Instruction.ComparisonType.LT => " < ",
                    GMCode.Bytecode.Instruction.ComparisonType.LTE => " <= ",
                    GMCode.Bytecode.Instruction.ComparisonType.EQ => " == ",
                    GMCode.Bytecode.Instruction.ComparisonType.NEQ => " != ",
                    GMCode.Bytecode.Instruction.ComparisonType.GTE => " >= ",
                    GMCode.Bytecode.Instruction.ComparisonType.GT => " > ",
                    _ => null
                };
                break;
        }
...
public class ASTFunction : ASTNode
{
    ...
    public bool Duplicated { get; set; }
    public bool NeedsParentheses { get; set; }
    public GMCode.Bytecode.Instruction.DataType DataType { get; set; } = GMCode.Bytecode.Instruction.DataType.Variable;
    public List<ASTNode> Children { get; set; }
...
public class ASTVariable : ASTNode
{
    ...
    public ASTNode Left;
    public GMCode.Bytecode.Instruction.VariableType VarType;
    public GMCode.Bytecode.Instruction.Opcode Opcode;

    public ASTVariable(GMVariable var, GMCode.Bytecode.Instruction.VariableType varType, GMCode.Bytecode.Instruction.Opcode opcode)
    {
        Variable = var;
        VarType = varType;
        Opcode = opcode;
...
public class ASTAssign : ASTNode
{
    ...
    public GMCode.Bytecode.Instruction.DataType DataType { get; set; } = GMCode.Bytecode.Instruction.DataType.Unset;
    public List<ASTNode> Children { get; set; }
    public GMCode.Bytecode.Instruction Compound;
    ...
    public ASTAssign(ASTVariable var, ASTNode node, GMCode.Bytecode.Instruction compound = null)
    {
        Children = new() { var, node };
        Compound = compound;
        CompoundKind = (compound == null) ? CompoundType.None : CompoundType.Normal;
    }
    public ASTAssign(GMCode.Bytecode.Instruction inst, ASTNode variable, bool isPrefix)
    {
        Compound = inst;
    ...
    public void Write(DecompileContext ctx, StringBuilder sb)
    {
        switch (CompoundKind)
        {
            case CompoundType.Normal:
                ...
                {
                    case GMCode.Bytecode.Instruction.Opcode.Add: sb.Append('+'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Sub: sb.Append('-'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Mul: sb.Append('*'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Div: sb.Append('/'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Mod: sb.Append('%'); break;
                    case GMCode.Bytecode.Instruction.Opcode.And: sb.Append('&'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Or: sb.Append('|'); break;
                    case GMCode.Bytecode.Instruction.Opcode.Xor: sb.Append('^'); break;
                }
                sb.Append("= ");
                Children[1].Write(ctx, sb);
                break;
            case CompoundType.Prefix:
                if (Compound.Kind == GMCode.Bytecode.Instruction.Opcode.Add)
                    sb.Append("++");
                else
                    sb.Append("--");
                Children[0].Write(ctx, sb);
            ...
            Children[0].Kind == ASTNode.StatementKind.Variable && Children[1].Kind == ASTNode.StatementKind.Binary)
        {
            ...
            if (bin.Children[0].Kind == ASTNode.StatementKind.Variable)
            {
                ...
                if (var.IsSameAs(Children[0] as ASTVariable))
                {
                    ...
                    if (ctx.Data.VersionInfo.FormatID < 15 || var.Opcode == GMCode.Bytecode.Instruction.Opcode.Push || var.Variable.VariableType == GMCode.Bytecode.Instruction.InstanceType.Self)
                    {
...
public class ASTAsset : ASTNode
{
    ...
    public bool Duplicated { get; set; }
    public bool NeedsParentheses { get; set; }
    public GMCode.Bytecode.Instruction.DataType DataType { get; set; } = GMCode.Bytecode.Instruction.DataType.Int32;
    public List<ASTNode> Children { get; set; }
