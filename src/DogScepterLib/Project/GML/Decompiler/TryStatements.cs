...
public class TryStatements
{
    ...
    public static List<TryStatement> FindAndClean(DecompileContext ctx)
    {
        ...
        foreach (Block b in ctx.Blocks.List)
        {
            ...
            else if (b.Instructions.Count >= 3 && b.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.B &&
                        b.Instructions[^2].Kind == GMCode.Bytecode.Instruction.Opcode.Popz)
            {
                GMCode.Bytecode.Instruction call = b.Instructions[^3];
                if (call.Kind == GMCode.Bytecode.Instruction.Opcode.Call)
                {
                ...
                if (pred.Branches[i] == tail)
                {
                    ...
                    if (pred.Kind == Node.NodeType.Block)
                    {
                        ...
                        if (predBlock.Instructions.Count >= 1 &&
                            predBlock.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.B)
                        {
            ...
            foreach (var pred in after.Predecessors)
            {
                if (pred.Kind == Node.NodeType.Block)
                {
                    ...
                    if (predBlock.Instructions.Count >= 2 && 
                        ...
                        predBlock.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.Popz &&
                        predBlock.Instructions[^2].Kind == GMCode.Bytecode.Instruction.Opcode.Call &&
