    ...
    public static List<SwitchStatement> Find(DecompileContext ctx)
    {
        ...
        for (int blockInd = ctx.Blocks.List.Count - 1; blockInd >= 0; blockInd--)
        {
            ...
            if (b.Instructions.Count >= 1 && !b.IsSwitchContinueBlock)
            {
                ...
                if (instr.Kind == GMCode.Bytecode.Instruction.Opcode.Popz)
                {
                    ...
                    if (b.Predecessors.Count == 1)
                    {
                        ...
                        // Do check for exit
                        if (b.Instructions.Count >= 2 && b.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.Exit)
                        {
                            bool allPopz = true;
                            for (int i = 1; i < b.Instructions.Count - 1; i++)
                                allPopz &= b.Instructions[i].Kind == GMCode.Bytecode.Instruction.Opcode.Popz;
                            if (allPopz)
                            {
                    ...
                    else
                    {
                        ...
                        if (b.Instructions.Count >= 2 && b.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.Exit)
                        {
                            ...
                            for (int i = 0; i < b.Instructions.Count - 1; i++)
                                if (b.Instructions[i].Kind != GMCode.Bytecode.Instruction.Opcode.Popz)
                                    shouldAbort = false;
                            if (shouldAbort)
                            {
                        ...
                        {
                            if ((b.Predecessors[i] as Block)?.LastInstr.Kind == GMCode.Bytecode.Instruction.Opcode.B)
                            {
                        ...
                        while (header.Index > 0)
                        {
                            ...
                            if (next.Instructions.Count >= 1 && next.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.B &&
                        ...
                        {
                            Block next = ctx.Blocks.List[header.Index - 1];
                            if (next.Instructions.Count >= 4 && next.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.Bt)
                            {
                        ...
                        if (endCasesBranch.Index + 1 < ctx.Blocks.List.Count)
                        {
                            ...
                            if (maybeRealEnd.Address < startCaseBodies && 
                                maybeRealEnd.Instructions.Count == 1 && maybeRealEnd.Instructions[0].Kind == GMCode.Bytecode.Instruction.Opcode.B)
                            {
    ...
    public static void InsertNode(DecompileContext ctx, SwitchStatement s)
    {
        ...
        else
        {
            ...
            for (int i = s.EndCasesBranch.Index + 1; i < end; i++)
            {
                ...
                if (curr.Instructions.Count >= 1 && curr.Instructions[^1].Kind == GMCode.Bytecode.Instruction.Opcode.B)
                {
            ...
            for (int i = 0; i < s.CaseBranches.Count; i++)
            {
                ...
                while (curr.Instructions[dupIndex].Kind != GMCode.Bytecode.Instruction.Opcode.Dup)
                    dupIndex--;
