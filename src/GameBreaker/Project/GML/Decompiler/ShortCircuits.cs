    ...
    public static List<ShortCircuit> Find(BlockList blocks, bool oldBytecode)
    {
        ...
        foreach (Block b in blocks.List)
        {
            if (b.Instructions.Count == 1)
            {
                ...
                if (instr.Kind == (oldBytecode ? GMCode.Bytecode.Instruction.Opcode.PushI : GMCode.Bytecode.Instruction.Opcode.Push) && instr.Type1 == GMCode.Bytecode.Instruction.DataType.Int16)
                {
        ...
        foreach (var s in ctx.ShortCircuitNodes)
        {
            ...
            while (curr != s.Tail)
            {
                if (curr.Kind == Node.NodeType.Block)
                {
                    ...
                    if (b.LastInstr.Kind == GMCode.Bytecode.Instruction.Opcode.B)
                    {
                        ...
                        if (b.Instructions.LastOrDefault()?.Kind == GMCode.Bytecode.Instruction.Opcode.B) // It might not be inside a loop!
