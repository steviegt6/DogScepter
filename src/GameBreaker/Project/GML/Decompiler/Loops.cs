    ...
    public static List<Loop> Find(BlockList blocks)
    {
        ...
        foreach (Block b in blocks.List)
        {
            ...
            if (b.Instructions.Count >= 1)
            {
                ...
                if (firstInstr.Kind == GMCode.Bytecode.Instruction.Opcode.PopEnv && !firstInstr.PopenvExitMagic)
                {
        ...
        foreach (var loop in ctx.LoopNodes)
        {
            ...
            while (work.Count != 0)
            {
                ...
                for (int i = 0; i < curr.Branches.Count; i++)
                {
                    ...
                    if ((branch.Address < loop.Address && branch.EndAddress <= loop.Address) || branch.Address >= loop.EndAddress)
                    {
                        ...
                        if (curr.Kind == Node.NodeType.Block && (curr as Block).LastInstr?.Kind == GMCode.Bytecode.Instruction.Opcode.B && 
                    ...
                    {
                        if (curr != loop.Tail &&
                            curr.Kind == Node.NodeType.Block && (curr as Block).LastInstr?.Kind == GMCode.Bytecode.Instruction.Opcode.B)
                        {
                            if (branch == loop.Tail && (curr as Block).ControlFlow == Block.ControlFlowType.Break)
                            {
                                ...
                                b.Instructions.FirstOrDefault()?.Kind == GMCode.Bytecode.Instruction.Opcode.Popz)
                            {
            ...
            switch (loop.LoopKind)
            {
                case Loop.LoopType.While:
                    ...
                    {
                        Node curr = work.Pop();
                        if (curr.Kind == Node.NodeType.Block && (curr as Block).LastInstr?.Kind == GMCode.Bytecode.Instruction.Opcode.Bf)
                        {
                ...
                case Loop.LoopType.With:
                    {
                        ...
                        if (loop.Tail.Instructions.Count == 2 &&
                            loop.Tail.Instructions[1].Kind == GMCode.Bytecode.Instruction.Opcode.B &&
                            loop.Tail.Index + 1 < ctx.Blocks.List.Count)
                        {
                            ...
                            if (loop.Tail.Branches[0].Address == nextBlock.EndAddress &&
                                ...
                                nextBlock.Instructions[0].Kind == GMCode.Bytecode.Instruction.Opcode.PopEnv &&
